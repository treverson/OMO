var contractAddress = "TXQuPUK4zxT17Pqg994x7j4wZpAXW8Lvq1"; //1
...
function updateUserInformation() {
    ...
    var totalPrice = 0;
	for (let i = 0; i < 110; i++) {
		tronconquestContract.ctry_(1, i).call().then(countryInfo => {
			let countryId = parseInt(countryInfo.id);
			let countryPrice = sunToDisplay(parseInt(countryInfo.price));
			let ownerAddress = tronWeb.address.fromHex(countryInfo.owner);
			if(ownerAddress === "TSBjPr73eLbBnsGQqm3yRz9y26jxRypgwi") {
				totalPrice = totalPrice + countryPrice;
			}
